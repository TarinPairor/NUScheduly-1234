...
interface FlashcardMainPageProps {
  ...
  knowItCards: any[];
  setKnowItCards: (cards: any[]) => void;
  dontKnowItCards: any[];
  setDontKnowItCards: (cards: any[]) => void;
}

export default function FlashcardMainPage({
  quizMode,
  selectedDeck,
  questionNumber,
  setQuestionNumber,
  ...
  const testModeIncrement = () => {
    if (
      questionNumber < selectedDeck.content.length - 1 &&
      isAnswered === true
    ) {
      setIsFinished(false);
      setQuestionNumber(questionNumber + 1);
    } else if (
      questionNumber > selectedDeck.content.length &&
      isAnswered === true
    ) {
      setFinishTest(true);
    }
